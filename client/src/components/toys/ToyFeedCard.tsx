  ...
  return (
    ...
    >
      ...
      <div className="relative aspect-[4/3] bg-gray-100 dark:bg-gray-800">
        ...
        {toy.condition && (
          <span className="absolute top-3 left-3 px-3 py-1 rounded-full text-xs font-medium bg-black/45 text-white backdrop-blur-sm dark:bg-black/45 light:bg-white/70 light:text-gray-900 z-10">
            {toy.condition}
          </span>
        )}
      ...
      <div className="p-3 space-y-1">
        ...
        <div className="flex items-center gap-2 text-xs text-gray-500 dark:text-gray-400">
          {toy.ageGroup && <span>{toy.ageGroup}</span>}
          {toy.ageGroup && toy.category && <span>·</span>}
          {toy.category && <span>{toy.category}</span>}
        </div>
        {toy.description && (
