-- ============================================================
-- ToyX Production Launch — Seed Script
-- Creates official platform account + example listings
-- Idempotent — safe to run multiple times
-- ============================================================
-- Run this AFTER production-cleanup.sql (if applicable)
-- Run this AFTER deploying the RC1 build (assets must exist)
-- ============================================================

BEGIN;

-- ── Step 1: Ensure account_type column exists ──
ALTER TABLE users ADD COLUMN IF NOT EXISTS account_type VARCHAR(32) NOT NULL DEFAULT 'standard';

-- ── Step 2: Upsert ToyX Official account ──
INSERT INTO users (
  ...
  'ToyX Official',
  '',
  '/assets/official/toyx-avatar-light.png',
...
Used for platform guidance, community education, and showcasing best-practice listings.',
  ...
  access_status = 'live',
  profile_image_url = '/assets/official/toyx-avatar-light.png',
...
-- ── Step 3: Create / backfill example listings ──

-- 3a. Space Explorer Action Figure Set
INSERT INTO toys (
...
SELECT
  ...
  ARRAY['/assets/official/space-explorer-1.png', '/assets/official/space-explorer-2.png'],
...
UPDATE toys SET
  image_urls = ARRAY['/assets/official/space-explorer-1.png', '/assets/official/space-explorer-2.png'],
  updated_at = NOW()
WHERE owner_id = 'official_toyx' AND name = 'Space Explorer Action Figure Set'
AND (image_urls IS NULL OR image_urls = ARRAY[]::text[] OR image_urls::text <> ARRAY['/assets/official/space-explorer-1.png', '/assets/official/space-explorer-2.png']::text);

-- 3b. Montessori Wooden Number Puzzle
INSERT INTO toys (
  owner_id, name, description, category, age_group, condition,
  location, image_urls, looking_for_categories, looking_for_details,
  is_available, created_at, updated_at
...
UPDATE toys SET
  image_urls = ARRAY['/assets/official/montessori-puzzle-1.png', '/assets/official/montessori-puzzle-2.png'],
  updated_at = NOW()
WHERE owner_id = 'official_toyx' AND name = 'Montessori Wooden Number Puzzle'
AND (image_urls IS NULL OR image_urls = ARRAY[]::text[] OR image_urls::text <> ARRAY['/assets/official/montessori-puzzle-1.png', '/assets/official/montessori-puzzle-2.png']::text);

-- 3c. Family Game Night Collection
INSERT INTO toys (
  owner_id, name, description, category, age_group, condition,
  location, image_urls, looking_for_categories, looking_for_details,
  is_available, created_at, updated_at
...
SELECT
  ...
  ARRAY['/assets/official/family-games-1.png', '/assets/official/family-games-2.png'],
...
UPDATE toys SET
  image_urls = ARRAY['/assets/official/family-games-1.png', '/assets/official/family-games-2.png'],
  updated_at = NOW()
WHERE owner_id = 'official_toyx' AND name = 'Family Game Night Collection'
AND (image_urls IS NULL OR image_urls = ARRAY[]::text[] OR image_urls::text <> ARRAY['/assets/official/family-games-1.png', '/assets/official/family-games-2.png']::text);
